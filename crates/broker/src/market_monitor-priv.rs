    ...
    P: Provider<Ethereum> + 'static + Clone,
{
    ...
    where
        P: Provider<Ethereum> + 'static + Clone,
    {
        tracing::info!("🎯 Starting FILTERED mempool monitoring for market: 0x{:x}", market_addr);
        ...
        loop {
                ...
                match Self::get_market_transactions_from_txpool(&provider, market_addr).await {
                    Ok(market_txs) => {
                        ...
                        for tx_hash in market_txs {
                            ...
                            // 🎯 CONFIG BAZLI FİLTRELEME İLE PROCESS ET
                            if let Err(e) = Self::process_market_tx_with_config_filter(
                                tx_hash,
                                provider.clone(),
                                market_addr,
                                config.clone(), // ← CONFIG PARAMETRESİ EKLENDİ
                            ).await {
            ...
            _ = cancel_token.cancelled() => {
                tracing::info!("🛑 Optimized mempool monitoring stopped");
                return Ok(());
            }
        }
        }
    ...
    //  3 FALLBACK METHOD - txpool_content başarısız olursa
            ...
            Ok(Some(block)) => {
                if let BlockTransactions::Hashes(tx_hashes) = block.transactions {
                    ...
                    for tx_hash in market_specific_txs {
                        ...
                        // FALLBACK METHOD güncelle
                        if let Ok(Some(tx)) = provider.get_transaction_by_hash(tx_hash).await {
                            if let Ok(Some(receipt)) = provider.get_transaction_receipt(tx_hash).await {
                                let tx_from = receipt.from;
                                let to_addr = tx.to(); // ← Bu Option<Address>

                                tracing::info!("📋 TX: 0x{:x} | From: 0x{:x} | To: {:?}",
                      tx_hash, tx_from, to_addr);

                                // Market contract kontrolü - Option kontrolü ekle
                                if let Some(to) = to_addr {
                                    if to == market_addr {
                                        tracing::info!("✅ Market contract'a giden tx");
                                        ...
                                        if tx_from == allowed_address {
                                            tracing::error!("🎯🚨 REQUESTOR TX FOUND! 0x{:x}", tx_hash);
                                            ...
                                            tracing::debug!("❌ From different address: 0x{:x}", tx_from);
                                        }
                                    }
                                }
                            }
